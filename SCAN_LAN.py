...
def scan_port(address, port):
    ...
    if s.connect_ex((address, port)) == 0:
        ...
        return False
    s.close()
    return address, requests.get("http://"+address+":"+str(config.app_port)+"/hostname").content

def get_internal_ip():
    s = os.popen("ifconfig").read()
    pattern = re.compile(r'inet (.+?) netmask')
    match_result = re.findall(pattern=pattern, string=s)
    if len(match_result) == 1:
        raise Exception("No network connected!")
    # internal_ip = None
    for ip in match_result:
        # if is_internal_ip(ip) and scan_port(ip, config.port):
